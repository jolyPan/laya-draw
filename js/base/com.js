...
function importCom() {
	...
	com.init = function(callback) {
		...
		function html_resize() {
			...
			if(dir == 'portrait') {
				...
				if(ibase.landscapeScaleMode == 'cover' || ibase.landscapeScaleMode == 'contain') {
					var size = imath.autoSize([ibase.landscapeWidth, ibase.landscapeHeight], [os.iphoneX?window.innerWidth-ibase.iphoneXOffsetLandscape*2:window.innerWidth, window.innerHeight], ibase.landscapeScaleMode);
					...
					article.css({
						...
						rotate: 0,
						scale: scale,
						x: (window.innerWidth / scale - ibase.landscapeWidth) * 0.5 + os.iphoneX?ibase.iphoneXOffsetLandscape/scale:0,
	...
	com.base64_send = function(base64, callback, secretkey) {
		if(base64) {
			secretkey = secretkey || 'test';
			$.post('http://tool.h5-x.com/cdn/base64', {
				data: base64,
				key: secretkey
			}, function(resp) {
				if(resp.errcode==0){
